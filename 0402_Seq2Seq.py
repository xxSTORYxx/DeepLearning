...
class Seq2Seq(nn.Module):
    ...
    def encode(self, x):
        ...
        hidden = (torch.zeros(1, x.shape[0], self.units), 
                torch.zeros(1, x.shape[0], self.units))
...
def train():
    ...
    # print("Chinese time order: yy/mm/dd", dataset.date_cn[:3], "\nEnglish time order: dd/M/yyyy", dataset.date_cn[:3])
    # print("Vocabularies: ", dataset.vocab)
    # print(f"x index sample: \n{dataset.idx2str(dataset.x[0])}",
    #     f"y index sample: \n{dataset.idx2str(dataset.y[0])}")
    # print(f'start_token={dataset.start_token}')
    # print(f'end_token={dataset.end_token}')
    ...
    for i in range(100):
        for batch_idx, batch in enumerate(loader):
            ...
            loss = model.step(bx, by) # train
            if batch_idx % 70 == 0:
                target = dataset.idx2str(by[0, 1:-1].data.numpy())
                # print(f'by={by}')
                # print(f'by[0, 1:-1]={by[0, 1:-1]}')
                # print(f'target={target}')
                pred = model.inference(bx[0:1]) # test, use 1 data in batch
                # print(f'bx={bx}')
                # print(f'bx[0:1]={bx[0:1]}')
                # print(f'pred={pred}')
